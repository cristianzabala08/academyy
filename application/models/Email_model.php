...
try {
    ...
    $mail->isSMTP();                                            // Send using SMTP  smtp.gmail.com
    $mail->Host       = get_settings('smtp_host');                // Set the SMTP server to send through
    $mail->SMTPAuth   = true;                                   // Enable SMTP authentication
    $mail->Username   = get_settings('smtp_user');                     // SMTP username      cristianzabala2014@gmail.com
    $mail->Password   = get_settings('smtp_pass');                               // SMTP password   zravrejqicwzvcwm
    $mail->SMTPSecure = 'ssl';                         // Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
    $mail->Port       = get_settings('smtp_port');                                    // TCP port to connect to, use 465 for `PHPMailer::ENCRYPTION_SMTPS` above

    //Recipients
    $mail->setFrom($from, get_settings('system_name'));
    $mail->addAddress($to);     // Add a recipient
	$mail->addReplyTo('No-reply-this@gmail.com', 'No reply');
